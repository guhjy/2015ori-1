...
In other words, widespread detection based on nonsignificant $p$-values does not seem promising. 
...
Moreover, the maximum absolute difference between the $SD_z$ and $max-min_z$ operationalizations is `r round(max(abs(dfvar$SD_z - dfvar$max_min_z)), 3)` and there is no reason to deviate from the original $SD_z$ operationalization as proposed by @. 
