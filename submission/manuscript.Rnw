...
for(i in 1:length(unique(ml3_dat$study_name)))
{
  ...
  binder <- data.frame(id = unique(ml3_dat$study_name)[i],
                       ...
                       std_congr_p = std_var(tmp$SDC, tmp$NC, iter = 100000),
                       std_incongr_p = std_var(tmp$SDI, tmp$NI, iter = 100000),
...
for (response in responses)
{
  ...
  p_mult_m_sd_congr <- ifelse(mean(df_ml3$mult_m_sd_congr) > mult_m_sd_congr,
                              sum(df_ml3$mult_m_sd_congr < mult_m_sd_congr) / length(df_ml3$mult_m_sd_congr),
                              sum(df_ml3$mult_m_sd_congr > mult_m_sd_congr) / length(df_ml3$mult_m_sd_congr))
  p_mult_m_sd_incongr <- ifelse(mean(df_ml3$mult_m_sd_incongr) > mult_m_sd_incongr,
                                sum(df_ml3$mult_m_sd_incongr < mult_m_sd_incongr) / length(df_ml3$mult_m_sd_incongr),
                                sum(df_ml3$mult_m_sd_incongr > mult_m_sd_incongr) / length(df_ml3$mult_m_sd_incongr))
  p_mult_m_m_across <- ifelse(mean(df_ml3$mult_m_m_across) > mult_m_m_across,
                              sum(df_ml3$mult_m_m_across < mult_m_m_across) / length(df_ml3$mult_m_m_across),
                              sum(df_ml3$mult_m_m_across > mult_m_m_across) / length(df_ml3$mult_m_m_across))
  p_mult_sd_sd_across <- ifelse(mean(df_ml3$mult_sd_sd_across) > mult_sd_sd_across,
                                sum(df_ml3$mult_sd_sd_across < mult_sd_sd_across) / length(df_ml3$mult_sd_sd_across),
                                sum(df_ml3$mult_sd_sd_across > mult_sd_sd_across) / length(df_ml3$mult_sd_sd_across))
  ...
  binder <- data.frame(id = response,
                       ...
                       std_congr_p = std_var(fab_dat$sd_congruent, fab_dat$congruent_trials, iter = 10000),
                       std_incongr_p = std_var(fab_dat$sd_incongruent, fab_dat$incongruent_trials, iter = 10000),
                       p_mult_m_sd_congr,
                       p_mult_m_sd_incongr,
                       p_mult_m_m_across,
                       p_mult_sd_sd_across)
