...
The collected data included \Sexpr{table(dat_summary$type)[1]} genuine data from Many Labs 1 \citep[\url{osf.io/pqf9r};][]{klein2014} and \Sexpr{table(dat_summary$type)[2]} fabricated datasets(\url{osf.io/e6zys}). \Sexpr{sum(dat_summary$type == 'qualtrics' & is.na(dat_summary$bonus))} participants did not participate for a bonus. Figure \ref{densities} shows a group-level comparison of the genuine- and fabricated $p$-values and effect sizes ($r$). Such group-level comparisons provide a general idea of the differences between the genuine- and fabricated data \citep[see also][]{Akhtar-Danesh2003}. 
...
<<AUC for variance methods, echo=FALSE, results=tex>>=
...
row_table <- c("Homogeneous combined",
                    ...
                    "Study 1 low condition",
                    "Study 1 high condition",
                    "Study 2 low condition",
                    "Study 2 high condition",
                    "Study 3 low condition",
                    "Study 3 high condition",
                    "Study 4 low condition",
                    "Study 4 high condition")
variances_table <- data.frame(Method = row_table, SDz = AUC[selsd], maxminz = AUC[selmaxmin])
names(variances_table) <- c("Method", "$SD_z$", "$maxmin_z$")
print(xtable(variances_table,
             ...
             label = "auc_variances", digits = 3), include.rownames = FALSE, sanitize.text.function=identity)
@
...
<<AUC for single fisher methods, echo=FALSE, results=tex>>=
sel <- grepl(Method, pattern = '^(Fisher test)')
row_table <- Method[sel]
tabl <- data.frame(Method = row_table, AUC = AUC[sel])
print(xtable(tabl,
             caption = "Area Under the Curve (AUC) for PLACEHOLDER",
             label = "auc_fisher", digits = 3), include.rownames = FALSE, sanitize.text.function=identity)
@

\subsubsection*{Performance of combining SDs and $p$-value methods to detect data fabrication}


<<AUC for combined fisher methods, echo=FALSE, results=tex>>=
sel <- grepl(Method, pattern = 'Combined Fisher test')
row_table <- Method[sel]
tabl <- data.frame(Method = row_table, AUC = AUC[sel])
print(xtable(tabl,
             caption = "Area Under the Curve (AUC) for PLACEHOLDER",
             label = "auc_combined", digits = 3), include.rownames = FALSE, sanitize.text.function=identity)
@
