...
for (pp in unique(dat$referrer))
{
  ...
  {
    study_nr <- j    
    indexor <- dat$referrer == pp & dat$study == study_nr
  ...
  ms_w <- c(rep(ms_w_1, 4), rep(ms_w_2, 4),
            rep(ms_w_3, 4), rep(ms_w_4, 4))
  sim_sigma <- NULL
  
  for (z in 1:16)
  {
    sim_sigma <- cbind(sim_sigma, ((rchisq(n = iter, df = n_min_1[z]) *  s2[z]) / n_min_1[z]) / ms_w[z])
  }
  
  sim_sigma_sd <- apply(sim_sigma, 1, sd)
  
  dat$variance_sd_p[sel] <- sum(sim_sigma_sd < obs_sigma_sd) / iter
...
dat_summary$fish_combine <- -2 * (log(dat_summary$variance_sd_p) + log(dat_summary$gender_fish_p) + 
        log(dat_summary$interaction_fish_p))
dat_summary$fish_combine_p <- pchisq(q = dat_summary$fish_combine,
